angular.module('Bidding').controller('SubmittedController', ['$timeout', '$scope', '$compile', '$location', '$state', 'BiddingAPI', 'globalValues', function ($timeout, $scope, $compile, $location, $state, BiddingAPI, globalValues) {
    ...
    $scope.$on('load-datatable', function () {
        $scope.bidsList = [];
        fillDataTable_Submitted();
    });

    var self = this;


    function fillDataTable_Submitted() {
        ...
        BiddingAPI.getDirectAwardsAndBids(globalValues.currentUserCompanyGuid).then(function (result) {
            ...
            for (var i = 0; i < result.vendorBids.length; i++) {
                bids_Submitted.bidGUID = result.vendorBids[i].bid.bidGUID;
                bids_Submitted.Client = result.vendorBids[i].lender.company.name;
                bids_Submitted.Requestor = result.vendorBids[i].lender.fullName;
                bids_Submitted.BidRequest = "test bid request date";
                bids_Submitted.BidDeadline = result.vendorBids[i].bid.proposedDateToComplete;
                bids_Submitted.Project = result.vendorBids[i].property.address.address1;
                bids_Submitted.Address = result.vendorBids[i].property.address.address1;
                bids_Submitted.ReportType = "test report type";
                bids_Submitted.Sites = "test site";


                $scope.bidsList.push(bids_Submitted);

            }
            console.log("obj: " + JSON.stringify($scope.bidsList));
            $(document).ready(function () {
                console.log("4");
                $scope.dataTable = $('#tableSubmitted_id').DataTable({
                    ...
                    "initComplete": function () {
                        ...
                        $('#tableSubmitted_id tbody tr').each(function () {
                            $(this).find('td:eq(0)').css('white-space', 'nowrap');
                        });



                        $('#tableSubmitted_id thead tr#filterrow th').each(function () {

                            var title = $('#tableSubmitted_id thead th').eq($(this).index() + 1).text();
                    ...
                    "columnDefs": [

                        {
                            ...
                            "searchable": false,
                            "sortable": false,
                            "defaultContent": "<a  class='retract-bid-Submitted pointer'>Retract Bid</a> | <a  class='update-bid-Submitted pointer'>Update Bid</a> "
                ...
                $('#tableSubmitted_id').on('click', 'a.retract-bid-Submitted', function (e) {
                ...
                $('#tableSubmitted_id').on('click', 'a.update-bid-Submitted', function (e) {
                ...
                $("#tableSubmitted_id thead input").on('keyup change', function () {
                ...
                function showContent() {
                    ...
                    var itemsTemplate = '<div class="row">' +
                        '<div class="col-md-6 col-xs-12" ng-repeat="item in submitted.columns_submitted">' +
                        '<input ng-model="item.visible" class="column-settings-checkbox" columnindex = "{{item.index}}"' +
                        'type="checkbox" ng-change="submitted.hideshowColumns(item)"></input> {{item.name}} ' + '</div> </div> </br>' +
                        '<div class="pull-right"><button close-popover class="btn-sm btn" ng-click="submitted.closeSettingsPopup()">Close</button>';
                    //                +
    ...
    this.settings = function () {
        ...
        for (var i = 0; i < $scope.dataTable.context[0].aoColumns.length; i++) {
            if ($scope.dataTable.context[0].aoColumns[i].title != "Action" && $scope.dataTable.context[0].aoColumns[i].title != "undefined" && $scope.dataTable.context[0].aoColumns[i].title != null && $scope.dataTable.context[0].aoColumns[i].title != "") {
                ...
                this.columns_submitted.push(this.obj);
            }
        }
        console.log("fghj" + JSON.stringify(this.columns_submitted));
