angular.module('Bidding').controller('NewRequestController', ['$timeout', '$scope', '$compile', '$location', '$state', 'BiddingAPI', 'globalValues', function ($timeout, $scope, $compile, $location, $state, BiddingAPI, globalValues) {
    ...
    $scope.$on('load-datatable', function () {
        ...
        fillDataTable_NewRequestData();
    });

    var self = this;


    function fillDataTable_NewRequestData() {
        //api call
        BiddingAPI.getDirectAwardsAndBids(globalValues.currentUserCompanyGuid).then(function (result) {
            ...
            for (var i = 0; i < result.vendorBids.length; i++) {
                bids_NewRequest.bidGUID = result.vendorBids[i].bid.bidGUID;
                bids_NewRequest.Client = result.vendorBids[i].lender.company.name;
                bids_NewRequest.Requestor = result.vendorBids[i].lender.fullName;
                bids_NewRequest.BidRequest = "test bid request date";
                bids_NewRequest.BidDeadline = result.vendorBids[i].bid.proposedDateToComplete;
                bids_NewRequest.Project = result.vendorBids[i].property.address.address1;
                bids_NewRequest.Address = result.vendorBids[i].property.address.address1;
                bids_NewRequest.ReportType = "test report type";
                bids_NewRequest.Sites = "test site";


                $scope.bidsList.push(bids_NewRequest);

            }
            // console.log("obj: " + JSON.stringify($scope.bidsList));
            if ($scope.bidsList != null && $scope.bidsList != "" && $scope.bidsList != undefined) {
                $(document).ready(function () {
                    //    console.log("4");
                    $scope.dataTable = $('#tableNewRequest_id').DataTable({
                        ...
                        "initComplete": function () {
                            ...
                            $('#tableNewRequest_id tbody tr').each(function () {
                                $(this).find('td:eq(0)').css('white-space', 'nowrap');
                            });



                            $('#tableNewRequest_id thead tr#filterrow th').each(function () {

                                var title = $('#tableNewRequest_id thead th').eq($(this).index() + 1).text();
                        ...
                        "columnDefs": [

                            {
                                ...
                                "searchable": false,
                                "sortable": false,
                                "defaultContent": "<a  class='submit-bid-NewRequests pointer'>Submit Bid</a> "
                    ...
                    $('#tableNewRequest_id').on('click', 'a.submit-bid-NewRequests', function (e) {
                    ...
                    $("#tableNewRequest_id thead input").on('keyup change', function () {
                    ...
                    function showContent() {
                        ...
                        var itemsTemplate = '<div class="row">' +
                            '<div class="col-md-6 col-xs-12" ng-repeat="item in newRequest.columns_newRequests">' +
                            '<input ng-model="item.visible" class="column-settings-checkbox" columnindex = "{{item.index}}"' +
                            'type="checkbox" ng-change="newRequest.hideshowColumns(item)"></input> {{item.name}} ' + '</div> </div> </br>' +
                            '<div class="pull-right"><button close-popover class="btn-sm btn" ng-click="newRequest.closeSettingsPopup()">Close</button>';
                        //                +
    ...
    this.settings = function () {
        ...
        for (var i = 0; i < $scope.dataTable.context[0].aoColumns.length; i++) {
            ...
            if ($scope.dataTable.context[0].aoColumns[i].title != "Action" && $scope.dataTable.context[0].aoColumns[i].title != "undefined" && $scope.dataTable.context[0].aoColumns[i].title != null && $scope.dataTable.context[0].aoColumns[i].title != "") {
                ...
                this.columns_newRequests.push(this.obj);
            }

        }
        console.log("fghj" + JSON.stringify(this.columns_newRequests));
