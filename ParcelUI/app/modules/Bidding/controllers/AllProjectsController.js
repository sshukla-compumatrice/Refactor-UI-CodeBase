angular.module('Bidding').controller('AllProjectsController', ['$timeout', '$scope', '$compile', '$location', '$state', 'BiddingAPI', 'globalValues', function ($timeout, $scope, $compile, $location, $state, BiddingAPI, globalValues) {

    $scope.bidsList = [];
    var bids_AllProjects = {
        "Client": "",
        "Requestor": "",
        "BidRequest": "",
        "BidDeadline": "",
    ...
    $scope.$on('load-datatable', function () {
        $scope.bidsList = [];
        fillDataTable_AllProjects();
    });

    var self = this;


    function fillDataTable_AllProjects() {
        ...
        BiddingAPI.getDirectAwardsAndBids(globalValues.currentUserCompanyGuid).then(function (result) {
            ...
            for (var i = 0; i < result.vendorBids.length; i++) {
                bids_AllProjects.Client = result.vendorBids[i].lender.company.name;
                bids_AllProjects.Requestor = result.vendorBids[i].lender.fullName;
                bids_AllProjects.BidRequest = "test bid request date";
                bids_AllProjects.BidDeadline = result.vendorBids[i].bid.proposedDateToComplete;
                bids_AllProjects.Project = result.vendorBids[i].property.address.address1;
                bids_AllProjects.Address = result.vendorBids[i].property.address.address1;
                bids_AllProjects.ReportType = "test report type";
                bids_AllProjects.Sites = "test site";


                $scope.bidsList.push(bids_AllProjects);

            }
            console.log("obj: " + JSON.stringify($scope.bidsList));
            $(document).ready(function () {
                console.log("4");
                $scope.dataTable = $('#tableAllProjects_id').DataTable({
                    ...
                    "initComplete": function () {
                        ...
                        $('#tableAllProjects_id tbody tr').each(function () {
                            $(this).find('td:eq(0)').css('white-space', 'nowrap');
                        });



                        $('#tableAllProjects_id thead tr#filterrow th').each(function () {

                            var title = $('#tableAllProjects_id thead th').eq($(this).index() + 1).text();
                ...
                $('#tableAllProjects_id').on('click', 'a.goToReport-AwaitingAcceptance', function (e) {

                 console.log("go to report");
                });

                $("#tableAllProjects_id thead input").on('keyup change', function () {
                ...
                function showContent() {
                    ...
                    var itemsTemplate = '<div class="row">' +
                        '<div class="col-md-6 col-xs-12" ng-repeat="item in allProj.columns_allProjects">' +
                        '<input ng-model="item.visible" class="column-settings-checkbox" columnindex = "{{item.index}}"' +
                        'type="checkbox" ng-change="allProj.hideshowColumns(item)"></input> {{item.name}} ' + '</div> </div> </br>' +
                        '<div class="pull-right"><button close-popover class="btn-sm btn" ng-click="allProj.closeSettingsPopup()">Close</button>';
                    //                +
    ...
    this.settings = function () {
        ...
        for (var i = 0; i < $scope.dataTable.context[0].aoColumns.length; i++) {
            if ($scope.dataTable.context[0].aoColumns[i].title != "Action" && $scope.dataTable.context[0].aoColumns[i].title != "undefined" && $scope.dataTable.context[0].aoColumns[i].title != null && $scope.dataTable.context[0].aoColumns[i].title != "") {
                ...
                this.columns_allProjects.push(this.obj);
            }
        }
        console.log("fghj" + JSON.stringify(this.columns_allProjects));
