/*
 * THIS SCRIPT HAS BEEN MODIFIED BY EDR. CHECK THE COMMENTS...
 * Versioning to .3 on 10/17/2014 to avoid browser caching issues.
 * 
 */
...
/*	MODIFIED BY EDR
 	moved this.suggestions definition here so it is global */
function AtDCore() {
...
/*	MODIFIED BY EDR
	moved the this.suggestions = []; definition to the AtDCore method so
	that it doesn't get overwritten each time. 
	Added param container_id so we can set the suggestions 
	for a specific element 
	Modified this.suggestions to be array of arrays	*/
AtDCore.prototype.processXML = function (responseXML, container_id) {
    ...
    for (var i = 0; i < errors.length; i++) {
        if (errors[i].getElementsByTagName('string').item(0).firstChild != null) {
            ...
            if (this.ignore_strings[errorString] == undefined) {
                ...
                this.suggestions[container_id].push(suggestion);
    ...
    return {
        errors: errorStruct,
        count: ecount,
        suggestions: this.suggestions[container_id]
    };
};

/* 	MODIFIED BY EDR
	Added param container_id for getting suggestions from
	specific element in the this.suggestions object */
AtDCore.prototype.findSuggestion = function (element, container_id) {
    ...
    var len = this.suggestions[container_id].length;

    for (var i = 0; i < len; i++) {
        var key = this.suggestions[container_id][i]["string"];

        if ((context == "" || context == this.suggestions[container_id][i]["context"]) && this.suggestions[container_id][i]["matcher"].test(text)) {
            errorDescription = this.suggestions[container_id][i];
            break;
        }
    }
...
AtDCore.prototype.removeWords = function (node, w) {
    ...
    this.map(this.findSpans(node).reverse(), function (n) {
        if (n && (parent.isMarkedNode(n) || parent.hasClass(n, 'mceItemHidden') || parent.isEmptySpan(n))) {
            ...
            } else if (!w || n.innerHTML.toLowerCase() == w.toLowerCase()) {
...
/*
 * MODIFIED BY EDR
 * Copied syncContents from AtD._checkTextArea here
 */
...
var AtD = {
    rpc: '',
    /* see the proxy.php that came with the AtD/TinyMCE plugin */
    rpc_css: 'http://www.polishmywriting.com/atd-jquery/server/proxycss.php?data=',
    /* you may use this, but be nice! */
    rpc_css_lang: 'en',
    api_key: '',
    i18n: {},
    listener: {}
...
AtD.clickListener = function (event) {
    ...
    if (AtD.core.isMarkedNode(event.target)) {
        ...
        var currentTargetId = $(event.target).closest('div').attr('id');
        AtD.suggest(event.target, currentTargetId);
    }
};

/*	MODIFIED BY EDR
	Added param container_id to call to AtD.core.processXML
	so that we can set the suggestions for individual 
	elements when doing a bulk spellcheck */
AtD.processXML = function (container_id, responseXML) {

    var results = AtD.core.processXML(responseXML, container_id);
    ...
    //jQuery('.hiddenSpellError').click(AtD.clickListener);
...
/*	MODIFIED BY EDR
	Added param container_id to define which container
	we need the suggestions for 
	
	Added calculations for suggestmenu placement
	*/
AtD.suggest = function (element, container_id) {
    /* construct the menu if it doesn't already exist */

    $(".suggestmenu").hide();
    AtD.container = container_id;
    var menuname = "suggestmenu_" + AtD.container;

    if (jQuery("#" + menuname).length == 0) {
        var suggest = jQuery('<div id="' + menuname + '" class="suggestmenu"></div>');
        suggest.prependTo('body');
    } else {
        var suggest = jQuery("#" + menuname);
        suggest.hide();
    }

    /* find the correct suggestions object */
    errorDescription = AtD.core.findSuggestion(element, container_id);
    ...
    /* show the menu */
    // element is the span of the hidden error, fyi
    var pos = jQuery(element).offset();
    var width = jQuery(element).width();
    var winWidth = jQuery("#Write-tab").width();
    var winHeight = jQuery("#Write-tab").height();
    var sugWidth = jQuery(suggest).width();
    var sugHeight = jQuery(suggest).height();
    var leftpos = (pos.left + width);
    var toppos = pos.top;

    // since our tables can go out of bounds of the viewport (yuck)
    // we may need to adjust the position of the suggest menu
    // do some math here to see if it needs adjusting.
    if ((pos.left + sugWidth) > winWidth) {
        leftpos = (pos.left - sugWidth);
    }

    if ((pos.top + sugHeight) > winHeight) {
        toppos = (pos.top - sugHeight);
    }
...
/*
 * MODIFIED BY EDR. 
 * The #AtD_sync_ div was not unique. Appended the id param to the div ID.
 */
AtD.restoreTextArea = function (id) {
    ...
    /* MODIFIED BY EDR
       This didn't work nicely since we aren't using the built in AtD
       buttons, so removing the check for link/before.
       */
...
/* 	MODIFIED BY EDR
	pulled this out of AtD._checkTextArea
	allows us to update the hidden div from
	other methods, like applySuggestion and 
	editSelection.
	*/
AtD.syncContents = function () {
    // force an update of the hidden input
    // depending on mouse and menu position
    // the syncContents method may not get called
    // so we replicate it here to be sure that it will.
    var div = jQuery('#' + AtD.container);
    var hidden = jQuery('#AtD_sync_' + AtD.container);
 ...
 * MODIFIED BY EDR. 
 * The #AtD_sync_ div was not unique. Appended the id param to the div ID.
 * Also commented out the call to restoreTextArea in the success block of
 * commChannel. It was causing the last entries in the error array to be 
 * removed when the suggestion was applied to another textarea.
 */
AtD._checkInlineCKEDitor = function (id, commChannel, linkId, after) {
...
AtD._checkTextArea = function (id, commChannel, linkId, after) {
    ...
    if (options['link'].val() != options['before']) {

        AtD.restoreTextArea(id);
        // maybe here we make it restore all textareas??
    } else {
        ...
        hidden.attr('id', 'AtD_sync_' + id);
        hidden.val(container.val());
        var name = container.attr('name');
        var style = "overflow: auto; white-space: pre-wrap; outline: none; color: rgb(85, 85, 85); font-size: 12px; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; border: 1px solid rgb(204, 204, 204); text-align: start; margin: 0px 0px 10px; width: 136px; line-height: 20px; left: auto; right: auto; top: auto; bottom: auto; position: static; padding: 4px 6px; height: 60px; background-color: rgb(255, 255, 255);";
        ...
        div.mouseleave(syncContents);
        div.mouseover(syncContents);
...
jQuery.fn.addProofreader = function (options) {
    ...
    return this.each(function () { // Ensures AtD has the correct data in its object.
        function activateAtD() {
            if (typeof AtD === "object") {
                AtD.rpc = 'http://refactorbridgeqa.parcelplatform.com/ajax/AtD/proxy.php?url=',
                    AtD.rpc_batch = 'http://refactorbridgeqa.parcelplatform.com/ajax/AtD/batchProxy.php?url=',
                    AtD.rpc_id = userName.replace(' ', '') + userID,
                    AtD.api_key = "parcel",
                    AtD.rpc_css = "http://refactorbridgeqa.parcelplatform.com/js/tinymce/3.5.10/plugins/AtD/css/content.css",
                    AtD.ignore_enable = "true",
                    AtD.ignore_strings = getIgnoredStrings(),
                    AtD.show_types = "Bias Language,Cliches,Complex Expression,Diacritical Marks,Double Negatives,Hidden Verbs,Jargon Language,Passive voice,Phrases to Avoid,Redundant Expression";
            }
        }

        activateAtD(); // Ensures AtD has the correct data in its object.
        function activateAtD() {
            if (typeof AtD === "object") {
                AtD.rpc = 'http://refactorbridgeqa.parcelplatform.com/ajax/AtD/proxy.php?url=',
                    AtD.rpc_batch = 'http://refactorbridgeqa.parcelplatform.com/ajax/AtD/batchProxy.php?url=',
        ...
        node.attr('class', 'AtD-textarea-button');
